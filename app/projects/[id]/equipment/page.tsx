...
export default function EquipmentListPage() {
  ...
  function downloadExcel() {
    ...
    fetch(`${apiBase()}/projects/${id}/export/excel`, {
      ...
      .then(async (res) => {
        ...
        a.href = url;
        a.download = `project_${id}_equipment_list.xlsx`;
        a.click();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-ink-500">Live MEL data — searchable, sortable, exportable.</p>
        </div>
        <div className="flex items-center gap-2">
          <button className="btn-secondary" onClick={downloadExcel}>
            <Download className="h-4 w-4" /> Excel
          </button>
          <Link className="btn-secondary" href={`/projects/${id}/equipment/import`}>
            <Upload className="h-4 w-4" /> Import Excel
          </Link>
          <Link className="btn-primary" href={`/projects/${id}/equipment/new`}>
      ...
      {data && <EquipmentTable projectId={id} rows={data} />}
    </div>
  );
}
